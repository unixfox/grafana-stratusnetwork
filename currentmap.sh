...
do
        ...
        do
            ...
            mysql -u $mysql_user -p$mysql_passwd stratusgraph -e "INSERT INTO recentmatches (\`id\`, \`When\`, \`Map\`, \`Length\`, \`Participants\`, \`Winner\`) VALUES ('8ce9e323-725c-47d6-bc2a-677a298cb6fa', '$when', \"$map_name\", '$length', '$participants', '$winner');";
        done
        sleep 4s
done
